...
namespace IFramework.Hotfix.AB
{
    class Collect
	{
        public static event Func<List<Collecter>> onLoadBuilders;
        private static Dictionary<string, List<string>> dps = new Dictionary<string, List<string>>();
        private static List<Collecter> collecters = new List<Collecter>();

        private static List<string> collectedAssets = new List<string>();
        private static List<AssetBundleBuild> abbuilds = new List<AssetBundleBuild>();

        public static List<AssetBundleBuild> GetCollection(string manifestPath)
        {
            ...
            build.assetBundleName = ABTool.helpABName;
        ...
        private static void CollectDependences()
        {
            for (int i = 0; i < collecters.Count; i++)
            {
                var paths = collecters[i].MeetFiles;
                ...
                foreach (var asset in assets)
                {
                    ...
                    if (importer is TextureImporter)
                    {
                        ...
                        if (ti.textureType == TextureImporterType.Sprite)
                        {
                            ...
                            {
                                var settings = ti.GetPlatformTextureSettings(ABTool.platformName);
                                settings.format = ti.GetAutomaticFormat(ABTool.platformName);
        ...
        public static void SetBuild( AssetBundleBuild build,IEnumerable<string> collectedassetNames)
        {
            collectedAssets.AddRange(collectedassetNames);
            abbuilds.Add(build);
        }
        ...
        public static List<string> GetNonCollect(Collecter collecter)
        {
